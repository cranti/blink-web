...
% > BLINK MOD: add threshold? (# consecutive frames necessary)
% > fix the input boxes: bigger font (Can I set defaults for
%   input/warning/error dialogs?)
...
% > add 'TooltipString' properties for some items
%
%
%LATER
...
uimenu(hYScroll, 'Label', 'Up',...
    'Callback', {@cbScrollPsth GUIDATA 'up'});
uimenu(hYScroll, 'Label', 'Down',...
    'Callback', {@cbScrollPsth GUIDATA 'down'});

hSortBy = uimenu(hPsthAxesMenu, 'Label', 'Sort targets by');
uimenu(hSortBy, 'Label', 'Original',...
    'Callback', {@cbScrollPsth GUIDATA 'sort_orig'});
uimenu(hSortBy, 'Label', 'Ascending density',...
    'Callback', {@cbScrollPsth GUIDATA 'sort_ascend'});
uimenu(hSortBy, 'Label', 'Descending density',...
    'Callback', {@cbScrollPsth GUIDATA 'sort_descend'});
...
uicontrol(hPermsAdvPanel,...
    ...
    'HorizontalAlignment', 'left',...
    'Units','pixels',...
    'Position',[10 135 230 25],...
    'BackgroundColor', inputPanelColor);

%W VALUE
wTip = 'Standard deviations considered for Gaussian kernel (optional)';
